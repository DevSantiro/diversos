...
if($act == 'calcular'){
   ...
   //CHAMADA DA FUNCAO
   $diasUteis = DiasUteis($data1, $data2);
   $diasNormal = CalculaDias($data1, $data2);
   /* 
   <?php echo $diasNormal; ?> dias entre <?php echo $DataInicial; ?> e <?php echo $DataFinal; ?> <br />
   <?php echo $diasUteis; ?> dias úteis entre <?php echo $DataInicial; ?> e <?php echo $DataFinal; ?> <br />
   */

   //$dados = [];

   //$dados[0][0] = 'Total de '. $diasNormal .' dias normais entre ' . $data1 . ' e ' . $data2;
   $dados =  'Total de '. $diasUteis .' dias uteis entre ' . $data1 . ' e ' . $data2;
...
<body>
   ...
   <label>Insira a Primeira Data</label>
      <input type="date" name="data1" id="data1">
   <label>Insira a Segunda Data</label>
...
<script>
   ...
   function Calcular() {
      ...
      data = new Date(data1);
      dataFormatada1 = data.toLocaleDateString('pt-BR', {timeZone: 'UTC'});

      data22 = new Date(data2);
      dataFormatada2 = data22.toLocaleDateString('pt-BR', {timeZone: 'UTC'});

      //alert(dataFormatada1);
      //alert(dataFormatada2);

      $.ajax({
          method: "POST",
          url: "",
          datatype: 'JSON',
          data: { act: "calcular", data1: dataFormatada1, data2: dataFormatada2 },
...
<!--  <?php   
   $DataInicial = "01/09/2020";
   $DataFinal = "30/09/2020";
